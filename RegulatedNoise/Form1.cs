            ...
            try{
                ...
                switch (e.InfoType){
                	...
                	case EDDN.RecievedEDDNArgs.enMessageInfo.Commodity_v1_Recieved:
                        ...
                        if(!(RegulatedNoiseSettings.UseEddnTestSchema ^ ((EDDN.Schema_v1)e.Data).isTest()))
                        {
                            Debug.Print("handle v1 message");
                            EDDN.Schema_v1 DataObject   = (EDDN.Schema_v1)e.Data;

                            // Don't import our own uploads...
                            if(DataObject.Header.UploaderID != RegulatedNoiseSettings.GetUserID()) 
                            { 
                	...
                	case EDDN.RecievedEDDNArgs.enMessageInfo.Commodity_v2_Recieved:
                        ...
                        // process only if it's the correct schema
                        if(!(RegulatedNoiseSettings.UseEddnTestSchema ^ ((EDDN.Schema_v2)e.Data).isTest()))
                        {
                ...
                if(DataRows != null && DataRows.GetUpperBound(0) >= 0)
                {
                    ...
                    bool isTrusty = Form1.RegulatedNoiseSettings.trustedSenders.Exists(x => x.Equals(name, StringComparison.InvariantCultureIgnoreCase));
